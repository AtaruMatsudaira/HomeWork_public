...
void rotate(char *fileName, int angle, int x = 0, int y = 0,
            double scale = 1.0) {
    ...
    center.x = image.cols / 2 + x; // 画像の中心
    center.y = image.rows / 2 - y;
    ...
    // 3列 の変換行列
    // dsize: 出力画像のサイズ 	flags: 補間手法 (default cv::INTER_LINEAR
    //(線形補間)) 	borderMode: 対応のとれない点を埋める方法 (default 0)
...
int main(int argc, char *argv[]) {
    char *args[6] = {argv[0], "sample.jpg", "0", "0", "0", "1"};
    if(argc >= 2)
        args[1] = argv[1]; //引数での入力画像指定
    if(argc >= 3)
        args[2] = argv[2]; //引数での角度指定
    if(argc >= 4)
        args[3] = argv[3]; //引数でのx座標
    if(argc >= 5)
        args[4] = argv[4]; //引数でのy座標
    if(argc >= 6)
        args[5] = argv[5]; //引数
    rotate(args[1], atoi(args[2]), atoi(args[3]), atoi(args[4]), stod(args[5]));
    return 0;
}
