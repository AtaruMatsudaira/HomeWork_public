...
inline void extractFilter(const int x, const int y, const cv::Mat &src,
                          Filter &dst);

//入力されたフィルタ2種類が同じかどうかを比較する関数
inline bool compareFilter(const Filter &p1, const Filter &p2,
                          const unsigned char wildNumber);

//削除される対象のピクセルであるかを確認する関数
inline bool isdeletePixel(const int x, const int y, const cv::Mat &src,
                          const vector<Filter> &deletePattern,
                          const vector<Filter> &undeletePattern);
...
int main(int argc, char *argv[]) {
    if(argc < 3) {
        cerr << "引数は3個以上必要です．" << endl
             << "使用例:" << argv[0] << " input.jpg output.jpg" << endl;
        return -1;
    }
    // 画像の読み込み
...
inline bool compareFilter(const Filter &p1, const Filter &p2,
    ...
    for(int i = 0; i < (int)p1.size(); i++) {
        for(int j = 0; j < (int)p1[i].size(); j++) {
            ...
            if(p1[i][j] != p2[i][j] && p1[i][j] != wildNumber &&
               p2[i][j] != wildNumber)
                return false;
        }
    }
...
引数    undeletePattern
:削除対象となったフィルタで，削除しない場合のフィルタパターン 戻り値
:trueは削除対象，falseは削除対象ではない
*/
inline bool isdeletePixel(const int x, const int y, const cv::Mat &src,
                          const vector<Filter> &deletePattern,
                          const vector<Filter> &undeletePattern) {
    ...
    for(int i = 0; i < (int)undeletePattern.size(); i++) {
        //ワイルドナンバーを127に設定して削除する場合のフィルタとマッチするかを確認する
        if(compareFilter(filter, undeletePattern[i], 127))
            return false;
    }
    return true;
}
...
inline void skeleton(const cv::Mat &src, cv::Mat &dst) {
    ...
    /*削除する場合のフィルタを格納した定数を定義
     * array[Y座標][X座標]でアクセス可能*/
    const vector<Filter> deleteFilter1{
        {{127, 255, 127}, {127, 0, 127}, {127, 127, 127}},
        {{127, 127, 127}, {127, 0, 255}, {127, 127, 127}}

    };
    const vector<Filter> deleteFilter2{
        {{127, 127, 127}, {127, 0, 127}, {127, 255, 127}},
        {{127, 127, 127}, {255, 0, 127}, {127, 127, 127}}

    };

    /*削除する場合の例外のフィルタを格納した定数を定義
     * array[Y座標][X座標]でアクセス可能*/
    const vector<Filter> undeleteFilter1{
        {{127, 255, 127}, {127, 0, 0}, {127, 0, 255}},
        {{255, 0, 127}, {0, 0, 255}, {127, 127, 127}},
        {{127, 127, 127}, {255, 0, 255}, {127, 0, 127}},
        {{127, 255, 127}, {0, 0, 127}, {127, 255, 127}},
        {{127, 0, 127}, {255, 0, 255}, {127, 127, 127}},
        {{127, 255, 127}, {127, 0, 0}, {127, 255, 127}},
        {{127, 127, 127}, {255, 0, 127}, {0, 255, 127}},
        {{0, 255, 127}, {255, 0, 127}, {127, 127, 127}},
        {{127, 255, 0}, {127, 0, 255}, {127, 127, 127}},
        {{127, 127, 127}, {127, 0, 255}, {127, 255, 0}},
        {{255, 0, 255}, {0, 0, 0}, {255, 127, 255}},
        {{255, 0, 255}, {127, 0, 0}, {255, 0, 255}},
        {{255, 127, 255}, {0, 0, 0}, {255, 0, 255}},
        {{255, 0, 255}, {0, 0, 127}, {255, 0, 255}}};
    const vector<Filter> undeleteFilter2{
        {{255, 0, 127}, {0, 0, 127}, {127, 255, 127}},
        {{127, 127, 127}, {255, 0, 0}, {127, 0, 255}},
        {{127, 127, 127}, {255, 0, 255}, {127, 0, 127}},
        {{127, 255, 127}, {0, 0, 127}, {127, 255, 127}},
        {{127, 0, 127}, {255, 0, 255}, {127, 127, 127}},
        {{127, 255, 127}, {127, 0, 0}, {127, 255, 127}},
    ...
    while(!end_flag) {
        ...
        for(int y = 1; y < verification.rows - 1; y++) {
            for(int x = verification.cols - 2; x >= 1; x--) {
                ...
                if((verification.ptr(y))[x] != 0)
                    continue;
                //削除条件してよい画素か確認
                if(isdeletePixel(x, y, verification, deleteFilter1,
                                 undeleteFilter1)) {
                    //注目画素を白画素に変換
                    (renewal.ptr(y))[x] = 255;
        ...
        for(int y = verification.rows - 2; y >= 1; y--) {
            for(int x = 1; x < verification.cols - 1; x++) {
                ...
                if((verification.ptr(y))[x] != 0)
                    continue;
                //削除条件してよい画素か確認
                if(isdeletePixel(x, y, verification, deleteFilter2,
                                 undeleteFilter2)) {
                    //注目画素を白画素に変換
                    (renewal.ptr(y))[x] = 255;
                    // 1つ以上の画素が変更されたので終了しない
    ...
    int count = 0;
    for(int x = 0; x < verification.cols; x++) {
        for(int y = 0; y < verification.rows; y++) {
            if(verification.at<cv::int32_t>((x, y)) == 0) {
                count++;
            }
        }
    }
